...
```{r}
sexV <- sub_UKBB$"f.22001.0.0" # retrieve sex column from data table
mSex <- 0
fSex <- 0
for (value in sexV) {
  if (value == "Male") {
...
cat("There are ", mSex, " males and ", fSex, "females in this dataset.")
```
...
**The responses to said question have been translated to binary 0 or 1 value indicating a positive or negative response.**
...
##There is an association because the R-squared value is .6926 which is not 0. It is appearing to be positive. Since the p-value is much less than 0.05, we reject the null hypothesiss and say that the association is significant.
...
```{r}
carsMPGG <- as.integer(auto_data$mpg > 23)
fitline <- glm(carsMPGG ~ weight, auto_data, family = binomial)
summary(fitline)
```
...
**If weight were 1 unit higher, the estimate for said intercept would increase by 11.449 units. This seems significant to me because a small change in weight does a lot to the model.**
